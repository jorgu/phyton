...
lag = input('Vilket lag? ')

antalSpeladematcher = 0

for row in myDataReader:
    hemmalag = row[2].lower()
    bortalag = row[3].lower()

    if hemmalag == lag.lower() or bortalag == lag.lower():
        antalSpeladematcher = antalSpeladematcher + 1

    if antalSpeladematcher > 0:
        meddelande = 'Lag ' + lag + ' spelade ' + str(antalSpeladematcher) + ' matcher!'
    else:
        meddelande = lag + ' spelade inga matcher! Har du angivit rätt lag?'
